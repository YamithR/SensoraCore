...
- **Procesamiento de Datos:** NumPy para cálculos básicos; regresión lineal propia (mínimos cuadrados)
- **Exportación:** openpyxl y pandas para generación de archivos Excel

#### 4.2 Componentes Principales

##### 4.2.1 Interfaz de Usuario (simpleAngle.ui)
```python
...
##### 4.2.2 Sistema de Calibración (Calibración guiada)
```python
...
##### 4.2.3 Hilo de Comunicación (SimpleAngleThread)
```python
...
- **Frecuencia de Muestreo:** 2 Hz (cada 0.5 s) según firmware actual
- **Rango de Medición:** -135° a +135° (configurable)
- **Resolución ADC:** 12 bits (0-4095) del ESP32
- **Filtrado de Señal:** No aplicado por defecto (opcional en futuras versiones)
...
- **Formato:** Excel (.xlsx) con múltiples hojas; CSV como alternativa
...
def perform_calibration(self):
    ...
    """
    # Implementación propia por mínimos cuadrados (sin scikit-learn)
    mean_x = sum(xs)/n
    mean_y = sum(ys)/n
    slope = sum((xs[i]-mean_x)*(ys[i]-mean_y) for i in range(n)) / sum((xs[i]-mean_x)**2 for i in range(n))
    intercept = mean_y - slope*mean_x
    r_squared = 1 - sum((ys[i]-(slope*xs[i]+intercept))**2 for i in range(n)) / sum((ys[i]-mean_y)**2 for i in range(n))
```
...
- **Comunicación TCP:** Verificación del handshake MODO:ANGULO_SIMPLE y flujo POT/ANG
...
- **Voltajes Seguros:** Máximo 3.3V en GPIO del ESP32 (use divisores si fuera necesario)
